...
const ScoreDisplay: React.FC<ScoreDisplayProps> = ({ score, darkMode }) => {
  ...
  return (
    <div className={`${darkMode ? 'bg-slate-800 text-white' : 'bg-white text-slate-800'} p-3 rounded-lg shadow-md flex items-center justify-between w-full md:w-auto`}>
      <div className="flex items-center">
        <TrendingUp size={18} className={darkMode ? 'text-blue-400' : 'text-blue-600'} />
        <span className="ml-2 font-medium text-sm">スコア:</span>
      </div>
