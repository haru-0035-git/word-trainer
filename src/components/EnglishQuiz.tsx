...
import wordsFromFile from "../data/english-words.json";
...
const Quiz: React.FC<QuizProps> = ({ darkMode }) => {
  ...
  useEffect(() => {
    if (selectedCategory) {
      ...
      // カテゴリが選択されていない (null の) 場合は全単語を表示
  ...
  const submitScore = async () => {
    ...
    const quizName = "english_quiz"; // より適切なクイズ名を検討
    ...
    try {
      ...
      alert(
        "スコアの送信に失敗しました。しばらくしてからもう一度お試しください。"
      );
    }
  };
