...
const getRandomChoices = (correct: Word, allWords: Word[], count = 4): string[] => {
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <div className="w-full max-w-2xl">
        ...
        <div className={`${darkMode ? 'bg-slate-800 shadow-indigo-900/20' : 'bg-white shadow-indigo-200/40'} shadow-xl rounded-xl p-6 md:p-8 w-full text-center transform transition-opacity duration-300 ${isAnimating ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
          {currentWord && (
            <>
              <div className="flex justify-center items-center mb-6">
                <h2 className={`text-2xl md:text-3xl font-bold ${darkMode ? 'text-white' : 'text-indigo-900'}`}>
                  <span className={`${darkMode ? 'text-blue-400' : 'text-blue-700'}`}>{currentWord.term}</span>
                </h2>
              </div>
              
              <p className={`text-lg font-semibold mb-6 ${darkMode ? 'text-slate-200' : 'text-slate-700'}`}>
                この用語の意味はどれ？
              </p>
